...
function BestOfYear() {
  const [bestOfYearGames, setBestOfYearGames] = useState([]);
  ...
  useEffect(() => {
    ...
    function initializeSwiper(containerSelector, swiperSelector) {
      const swiper = new Swiper(containerSelector, {
        ...
        navigation: {
          nextEl: `${swiperSelector} .best-of-year-custom-next-button`,
          prevEl: `${swiperSelector} .best-of-year-custom-prev-button`,
        },
      });

      const prevButton = document.querySelector(
        `${swiperSelector} .best-of-year-custom-prev-button`
      );
      const nextButton = document.querySelector(
        `${swiperSelector} .best-of-year-custom-next-button`
      );
    ...
    if (cachedBestOfYearGames) {
      // If game cards are already cached, load them
      setBestOfYearGames(cachedBestOfYearGames);

      initializeSwiper(".best-of-year", ".best-of-year");
    } else {
      ...
      (async () => {
        try {
          const [bestOfYearResponse] = await Promise.all([
          ...
          setBestOfYearGames(filteredBestOfYearGames);
  ...
  return (
    <div className="swiper-container best-of-year">
      <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          <button className="best-of-year-custom-prev-button bg-slate-600/50 text-3xl my-4 p-2 h-14 rounded-lg hover:scale-105 hover:bg-slate-600/80 dark:bg-white/60 dark:hover:bg-white/80">
            <svg
          ...
          <button className="best-of-year-custom-next-button bg-slate-600/50 text-3xl p-2 my-4 ml-5 h-14 rounded-lg hover:scale-105 hover:bg-slate-600/80 dark:bg-white/60 dark:hover:bg-white/80 ">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              className="h-8 w-8 font-extrabold antialiased"
      ...
      <div className="swiper-wrapper">
        ...
        {bestOfYearGames.map((game, index) => {
